...
def generate_consolidated_dashboard(consolidated_data):
    ...
    """
    # Define Reporting Period (Weekly, Tuesday to Tuesday)
            ...
            for category in df['WorkItemCategory'].unique():
                ...
                if not finished.empty:
                    ...
                    for _, row in finished.iterrows():
                        ...
                        if enh_eff is not None:
                            effs_adjusted.append(enh_eff)
                    
                    avg_efficiency = np.mean(effs_simple) if effs_simple else 0
                    avg_efficiency_adjusted = np.mean(effs_adjusted) if effs_adjusted else 0
                else:
                    avg_lead_time = 0
                    p85_lead_time = 0
                    avg_efficiency = 0
...
def calculate_enhanced_efficiency(row, wait_columns):
    ...
    try:
        ...
        # Convert blocked_days to numeric if it's not
        try:
            if pd.notna(blocked_days):
                blocked_days = int(blocked_days) if not isinstance(blocked_days, (int, float)) else blocked_days
            else:
                blocked_days = 0
        except:
            blocked_days = 0
    ...
    for projeto, df in consolidated_data.items():
        ...
        for category in df['WorkItemCategory'].unique():
            ...
            if not df_cat.empty:
                ...
                avg_blocked_days = df_cat['Blocked Days'].dropna().mean() if len(df_cat['Blocked Days'].dropna()) > 0 else 0
                avg_wait_days = df_cat['Wait Days'].dropna().mean() if len(df_cat['Wait Days'].dropna()) > 0 else 0
                
                metrics_list.append({
                    'Projeto': projeto,
                    'Tipo': category,
                    'Cycle Time Médio (dias)': round(df_cat['Cycle Time (dias)'].mean(), 2),
                    'Cycle Time Mediano (dias)': round(df_cat['Cycle Time (dias)'].median(), 2),
                    'Tempo Backlog Médio (dias)': round(df_cat['Tempo Backlog (dias)'].mean(), 2),
                    'Tempo 1º Movimento Médio (dias)': round(df_cat['Tempo 1º Movimento (dias)'].mean(), 2),
...
def generate_efficiency_wait_time_analysis(consolidated_data):
    ...
    for projeto, df in consolidated_data.items():
        ...
        for idx, row in df_complete.iterrows():
            ...
            blocked_days = row.get('Blocked Days', 0)
            try:
                blocked_days = int(blocked_days) if pd.notna(blocked_days) else 0
            except:
                blocked_days = 0
...
input_folder = r'C:\Users\W1 TI\OneDrive - W1\Documentos\Dados'
